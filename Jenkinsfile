...
pipeline {
    ...
    environment {
        ...
        PYTHON_IMAGE_VERSION = "3.11.5" //https://hub.docker.com/_/python/tags
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if ("${list[i]}" == "'UnitTests'" && env.ACTION == 'DEPLOY') {
                            stage('Unit Tests') {
                                print(list[i])
                                // stage details here
                                sh 'docker run -v "$WORKSPACE":/app -w /app python:$PYTHON_IMAGE_VERSION /bin/sh unitTest.sh'
                            }
                        } else if ("${list[i]}" == "'SonarQubeScan'" && env.ACTION == 'DEPLOY' && stage_flag['sonarScan']) {
                            stage('SonarQube Scan') {
                                // stage details here
                                 def sonar_org = "${metadataVars.sonarOrg}";
                                 def sonar_project_key = "${metadataVars.repoName}";
                                 if(sonar_org != null && sonar_org != '' && sonar_org != 'default-organization') {
                                    ...
                                    if (env.SONAR_CREDENTIAL_ID != null && env.SONAR_CREDENTIAL_ID != '') {
                                        withCredentials([usernamePassword(credentialsId: "$SONAR_CREDENTIAL_ID", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                                            sh """docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:4.7.0 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.projectKey="${sonar_project_key}"  -Dsonar.sources=. -Dsonar.projectName="${sonar_project_key}" -Dsonar.host.url="${metadataVars.sonarHost}" -Dsonar.organization="${metadataVars.sonarOrg}" -Dsonar.login=$PASSWORD"""
                                        }
                                    }
                                    else{
                                        withSonarQubeEnv('pg-sonar') {

                                            sh """docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:4.7.0 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.projectKey="${sonar_project_key}"  -Dsonar.sources=. -Dsonar.projectName="${sonar_project_key}" -Dsonar.organization="${metadataVars.sonarOrg}" -Dsonar.host.url="$SONAR_HOST_URL" -Dsonar.login=$SONAR_AUTH_TOKEN"""
                                        }
                                    }
                            }
                        }
                                ...
                                if (env.ACTION == 'DEPLOY' || env.ACTION == 'PROMOTE' || env.ACTION == 'ROLLBACK') {
                                    ...
                                    if (env.DEPLOYMENT_TYPE == 'KUBERNETES' || env.DEPLOYMENT_TYPE == 'OPENSHIFT') {
                                        ...
                                        if (env.DEPLOYMENT_TYPE == 'OPENSHIFT') {
                                          ...
                                          sh '''
                                                  ...
                                                  then
                                                    ...
                                                    sudo cp "$KUBECONFIG" config
                                                    sudo chown -R `id -u`:`id -g` config                   
                                                    docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION create ns "$namespace_name" || true
                                                    docker run --rm  -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config -v "${PWD}"/:/apps -w /apps $OC_IMAGE_VERSION oc adm policy add-scc-to-user anyuid -z $ACCOUNT -n "$namespace_name"
                                                    sudo rm config
                                        ...
                                        if (env.ARTIFACTORY == 'JFROG') {
                                            withCredentials([file(credentialsId: "$KUBE_SECRET", variable: 'KUBECONFIG'), usernamePassword(credentialsId: "$ARTIFACTORY_CREDENTIALS", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                                                sh '''
                                                    ...
                                                    sudo chown -R `id -u`:`id -g` config          
                                                    docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION create ns "$namespace_name" || true
                                                    docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION -n "$namespace_name" create secret docker-registry regcred --docker-server="$JFROG_LOGIN_URL" --docker-username="\"$USERNAME\"" --docker-password="\"$PASSWORD\"" || true
                                                    sudo rm config
                                                '''
                                        ...
                                        if (env.ARTIFACTORY == 'ACR') {
                                            withCredentials([file(credentialsId: "$KUBE_SECRET", variable: 'KUBECONFIG'), usernamePassword(credentialsId: "$ARTIFACTORY_CREDENTIALS", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                                                sh '''
                                                    ...
                                                    sudo chown -R `id -u`:`id -g` config      
                                                    docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION create ns "$namespace_name" || true
                                                    docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION -n "$namespace_name" create secret docker-registry regcred --docker-server="$ACR_LOGIN_URL" --docker-username="\"$USERNAME\"" --docker-password="\"$PASSWORD\"" || true
                                                    sudo rm config
                                                  '''
                                            }
                                        }
                                        withCredentials([file(credentialsId: "$KUBE_SECRET", variable: 'KUBECONFIG')]) {
                                           ...
                                           '''
                                            script {
                                                env.HELM_RELEASENAME = "${metadataVars.helmReleaseName}"
                                            }                            

                                            sh '''
                                                sudo cp "$KUBECONFIG" config
                                                sudo chown -R `id -u`:`id -g` config          
                                                docker run --rm  -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config -v "${PWD}"/:/apps -w /apps $HELM_IMAGE_VERSION upgrade --install "$HELM_RELEASENAME" -n "$namespace_name" helm_chart --atomic --timeout 300s --set image.repository="$REGISTRY_URL" --set image.tag="$BUILD_TAG" --set image.registrySecret="regcred"  --set service.internalport="$SERVICE_PORT" -f Helm.yaml
                                                sudo rm config
                                            '''
                                            ...
                                            script {
                                                ...
                                                env.temp_service_name = "${metadataVars.repoName}-$service".take(63)
                                                def url = sh (returnStdout: true, script: '''docker run --rm --user root -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config $KUBECTL_IMAGE_VERSION get svc -n "$namespace_name" | grep "$temp_service_name" | awk '{print $4}' ''').trim()
                                                if (url != "<pending>") {
                                                    print("##\$@\$ http://$url ##\$@\$")
                                                } else {
                            ...
                            stage('Destroy') {
                                ...
                                if (env.DEPLOYMENT_TYPE == 'KUBERNETES' || env.DEPLOYMENT_TYPE == 'OPENSHIFT') {
                                    withCredentials([file(credentialsId: "$KUBE_SECRET", variable: 'KUBECONFIG')]) {
                                        ...
                                        sudo cp "$KUBECONFIG" config
                                        sudo chown -R `id -u`:`id -g` config          
                                        docker run --rm  -v "${PWD}"/:/.kube/ -e KUBECONFIG=/.kube/config -v "${PWD}"/:/apps -w /apps $HELM_IMAGE_VERSION uninstall "$HELM_RELEASENAME" -n "$namespace_name"
                                        sudo rm config
                                        '''
                                    }
                                }
