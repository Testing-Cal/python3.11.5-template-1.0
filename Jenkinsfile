...
pipeline {
    ...
    environment {
        ...
        PYTHON_IMAGE_VERSION = "python:3.11-slim" //https://hub.docker.com/_/python/tags
                                 ...
                                 def sonar_project_key = "${metadataVars.repoName}";
                                    ...
                                    if (env.SONAR_CREDENTIAL_ID != null && env.SONAR_CREDENTIAL_ID != '') {
                                        withCredentials([usernamePassword(credentialsId: "$SONAR_CREDENTIAL_ID", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                                            sh """docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:11.0 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.projectKey="${sonar_project_key}"  -Dsonar.sources=. -Dsonar.projectName="${sonar_project_key}" -Dsonar.host.url="${metadataVars.sonarHost}" -Dsonar.organization="${metadataVars.sonarOrg}" -Dsonar.login=$PASSWORD"""
                                        }
                                    }
                                    else{
                                        withSonarQubeEnv('pg-sonar') {

                                            sh """docker run -v "$WORKSPACE":/app -w /app sonarsource/sonar-scanner-cli:11.0 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.projectKey="${sonar_project_key}"  -Dsonar.sources=. -Dsonar.projectName="${sonar_project_key}" -Dsonar.organization="${metadataVars.sonarOrg}" -Dsonar.host.url="$SONAR_HOST_URL" -Dsonar.login=$SONAR_AUTH_TOKEN"""
                                        }
                                    }
                            }
                        }
